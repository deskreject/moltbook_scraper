...
0. **[NEW — session 30] Change-based-panel value & format.** First-pass assessment (session-30 log §3): the change-driven panels are the claimed edge over the Observatory, but as of 2026-05-29 only ~1.5-2.3% of entities have ≥2 observations (6 runs / ~4 weeks since the Phase-3/4 writer started) — currently near-cross-sectional. Where change exists the signal is real and heavy-tailed (agent karma spread avg 239 / max 237,757). Format (tidy event-sourced panels + transition log) is sound. Two things to decide once it matures / before publishing on it: (a) is the **4-week post-metrics age cutoff + weekly cadence** too sparse for early-vote-dynamics (~4 points/post)? (b) event-writer bugs (item #2 above) must be fixed or the events tables are unusable. Re-run the §3 richness queries periodically to track maturation.
...
2. **Event-writer correctness sweep [escalated session 30]** — three symptoms now confirmed against the live DB: `agent_events = 0` AND `submolt_events = 0` across 6 runs (is_claimed / submolt transition writers emitting nothing — probe `SELECT COUNT(*) FROM agents WHERE is_claimed_first != is_claimed;` etc.); and `post_events.is_spam = 286,009` (~10% of posts, vs 48 is_pinned) which smells like a boolean-comparison writer bug (CLAUDE.md "Alert C"). **This now also gates the change-based-data research value (see below) — fix before relying on any event table.**
