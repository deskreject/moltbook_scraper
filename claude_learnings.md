...
**`fetch_comments_only` `except Exception: return []` masked rate-limit-induced data corruption for ~3 days (session 26, 2026-05-08; empirically classified session 29, 2026-05-17).**
- The silent except was flagged in `readme_api_limit.md` line 85 back in session 8 (March 2026) and never fixed. May 5 monthly tripped the infra-layer per-IP rate limit during a multi-day comments-full run; every 429 became `[]`, and `_detect_deleted_comments` tombstoned 4,126 comments across 521 posts + 6,428 posts that were not all actually deleted.
...
- **Empirical classifier (session 29):** the corruption is bimodally separable. Per-affected-post `tombstoned / in_db_total` is either **≥ 0.8** (whole-post wipe, n=324 posts, 1,533 comments — sampled recovery 28/28 = 100%, **rate-limit-induced**) or **< 0.5** (partial tombstoning, n=197 posts, 2,593 comments — sampled recovery 0/546 = 0%, **genuine deletions**). Zero posts fall in [0.6, 0.8). The discriminator `tombstoned == in_db AND in_db > 0` cleanly isolates false positives. Hi-stratum posts are uniformly small (104 had 1 comment, 134 had 2-4, 0 had ≥100) because partial-tombstoning requires the API to return *some* content — big threads can't end up in the rate-limit bucket.
- **Same bug class is in the posts-stage path (session 29).** `client.py:fetch_posts_streaming` catches every Exception including `RateLimitError`, counts consecutive errors, and after 10 silently breaks the loop. `scraper.py:scrape_posts` then marks `all_db_ids - seen_ids` as deleted against an incomplete seen_ids. If pagination dies near the end of the walk, you get a small false-positive set indistinguishable from a real deletion run. `fetch_submolts` has the same swallow pattern but no downstream `--detect-deletions` consumer for submolts — structurally fragile, not currently corrupting.
- **Resolution before re-enabling monthly:** `fetch_comments_only` and `fetch_posts_streaming` must distinguish `RateLimitError` from "empty result"; `_detect_deleted_comments` must refuse to tombstone when `api_comment_ids` is empty AND `existing_ids` is non-empty (defensive guard catches the failure mode regardless of upstream).
- Full diagnostic trace (signature checklist for recognizing recurrence vs. something new, kill order, damage quantification, rate-limit probes) in `CLAUDE/session_logs/2026_05_08_session_log.md`. Empirical classifier + posts-stage bug discovery in `CLAUDE/session_logs/2026_05_17_session_log.md`. Current state and resume plan in `claude_handover.md`.
...
**`responses` mock repeats its last registered response → paginating client methods loop forever in tests (session 30, 2026-05-29).**
- A new integration test drove `fetch_submolts()` with two mocked responses (429, then a 200 returning a non-empty `submolts` list). `responses` replays the last registered response indefinitely, so `fetch_submolts`'s `while True` pagination never hit an empty page → infinite loop, ~1.5 GB RAM and climbing. Both background pytest runs auto-backgrounded (it was the loop, not slow cold-start) and had to be killed mid-run (6 passing dots, no summary line).
- **Resolution:** never drive a *paginating* client method from a `responses` test expecting it to terminate. Test the single-request path (`client._request(...)` directly) so exactly the registered responses are consumed. Fixed in `tests/test_client.py::TestBackoffDelay::test_429_with_retry_after_honored_end_to_end`. See `CLAUDE/session_logs/2026_05_29_session_log.md` §5.

**Proposed a "fix" for behavior that was already correct (session 22).**
- Drafted a backup-prune bug fix and a weekly-vs-weekly lock addition, both framed as "ship before Apr 27 cron". User asked what the fix actually did. Re-reading `scripts/weekly_scrape.sh` end-to-end showed the lock was already there (`/tmp/moltbook_scrape.lock` line 19, PID-based since session 12) and the prune step was already there (line 137+, runs at end of weekly — just hadn't fired yet because the Apr 20 weekly was still mid-enrich). Both "fixes" would have been pure bloat.
- **Resolution:** Before writing a fix, read the target script top-to-bottom — not just the section that "looks relevant". Confirm the described bug by observing it, not by inferring it from a symptom (Apr 13 + Apr 20 backups both present ≠ prune broken; could mean prune hasn't run yet).
